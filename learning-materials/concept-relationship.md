...
【TODO：请用自己的话改写本节，这是评分标准中"体现自己的判断"的直接依据。以下为草稿】

- 草稿：三个概念其实是同一件事的三个层面——让模型可靠地完成复杂工作。上下文回答"信息从哪来"，Agent 回答"怎么持续行动"，Skill 回答"经验怎么不流失"。我感觉最反直觉的一点是：上下文不是"越大越好"的免费资源，而像内存一样需要精细管理；Skill 之所以成立，正是因为上下文稀缺，才需要"按需加载"的知识形式。另外我认为三者有依赖顺序：不懂上下文机制，就理解不了 Skill 为什么要设计成渐进加载；不懂 Agent 的失控风险，就理解不了 Skill 里为什么要写自检清单。
